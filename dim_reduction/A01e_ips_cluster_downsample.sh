#!/bin/bash
#$ -cwd
#$ -o logs/A01e_ips_cluster_downsample.$JOB_ID.$TASK_ID
#$ -j y
#$ -N A01e_ips_cluster_downsample
#$ -l h_data=4G,h_rt=18:00:00,h_vmem=64G
#$ -pe shared 12
#$ -t 1-2:1
...
RESOLUTIONS=1000,5000,10000,25000,100000,250000,500000,1000000
BALANCE_RES="1000 5000 10000 25000 100000 250000 500000 1000000"
NCPUS=12
...
# ID=1
ID=$SGE_TASK_ID
SAMPLE=$(sed -n "${ID}p" $PROJDIR/txt/${PSEUDO}.txt)
echo "Sample: $SAMPLE"

# CONTACT_LIST=$PROJDIR/txt/pseudobulk/${PSEUDO}/${SAMPLE}.txt
CONTACT_LIST=$PROJDIR/txt/pseudobulk/downsample/${PSEUDO}/${SAMPLE}.txt
...
# otherwise juicer pre fails with "chromosome combination appears in multiple blocks".
